...
- GitHub CLI (`gh`) when publishing test releases to GitHub.
...
```text
src-tauri/target/release/bundle/nsis/IDC Cupid_<version>_x64-setup.exe
```
...
5. Create a SHA256 checksum. GitHub normalizes asset names by replacing spaces with dots, so write the checksum file against the downloaded asset name.
   ...
   ```powershell
   $version = "0.1.2"
   $installer = "src-tauri\target\release\bundle\nsis\IDC Cupid_${version}_x64-setup.exe"
   $checksum = "src-tauri\target\release\bundle\nsis\IDC.Cupid_${version}_x64-setup.exe.sha256"
   $hash = (Get-FileHash -LiteralPath $installer -Algorithm SHA256).Hash.ToLowerInvariant()
   "$hash  IDC.Cupid_${version}_x64-setup.exe" | Set-Content -LiteralPath $checksum -Encoding ascii
...
6. Write release notes. Keep them short and include:
   ...
   - a link back to [docs/desktop-install-guide.md](docs/desktop-install-guide.md)
...
7. Commit, tag, and push.
   ...
   git tag -a "v${version}" -m "IDC Cupid v${version} test release"
...
8. Create the GitHub prerelease and upload the installer plus checksum.
   ...
   gh release create "v${version}" $installer $checksum `
     --repo GabrielHub/idc `
     --title "IDC: Cupid v${version} test alpha" `
     --notes-file $notes `
     --prerelease
   ```
...
9. Verify the published release.
   ...
   Get-Content -LiteralPath (Join-Path $dir "IDC.Cupid_${version}_x64-setup.exe.sha256")
   ```
...
There is no `AI_GATEWAY_API_KEY` fallback. Gateway keys are entered by the player and stored outside the game save through `app/services/ai/gateway-key-store.ts`. Browser builds use the `idc.cupid.aiGatewayKey` localStorage key. Desktop builds use `secrets/gateway-api-key.txt` under the Tauri app local data directory, and migrate any older browser-storage key into that file. The game save stores provider choice, model IDs, base URLs, and `aiSetupComplete`, but not the Gateway key.
...
`vp run tauri:build` is the release gate for desktop packaging because it includes `vp check`, type generation, TypeScript, tests, desktop bundle verification, and Tauri packaging.
