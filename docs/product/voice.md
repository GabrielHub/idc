...
This document owns IDC voice, prose mechanics, and comedic engine. `app/app.css` and `docs/product/visual-design.md` own the frontend visual language; `docs/product/image-style.md` owns image asset style. Together they define the world feel.
...
### Member Voice Fixture Contract

Every member fixture needs a `voice` block with these five fields:
...
A member without a voice block is not ready to ship. The LLM character performer reads this block at prompt time. Vague voice blocks produce house-style mush. Use `docs/workflows/add-member.md` for the full member workflow.

Voice fields do not score gameplay. Use `docs/product/gameplay-traits.md` for hidden match tags, authored boundaries, filed reads, and boundary risk authoring. Use `docs/workflows/add-member.md` for the full authoring checklist. A future member needs both: voice for how they talk, gameplay tags for booking pressure, and player knowledge rules for what Cupid may show.
...
The LLM owns the reaction. Authoring can make a reaction likely through bio, profile, needs, dealbreakers, secrets, and voice, but it must not script whether a member believes, rejects, mocks, accepts, or falls for another member's frame. A modern human may read a non-human as a costume, a liar, a bit, a miracle, or a strangely sincere date. A non-human may read modern confusion as rude, provincial, charming, or irrelevant. The date transcript decides.

Do not point one member fixture at another specific member. A member file should never say that Cassie hates Reaver, that Jenna is for Ryan, or that Vhool is bad for a named person. Put the reusable pressure in the member's own design: what they want, what they protect, what language they use, what they refuse, and what they believe Cupid is. The same rule applies to scenarios and scenario events. A scenario may imply a hook, such as Olympus, a camera, a bargain, or a rescue poster, but it must not name the member it is for.

Character prompts must feed the member's reason for using Cupid and their reality frame. The model needs to know whether the member thinks Cupid is a normal app, a sanctioned trial, a competitor, a consort marketplace, a handler routed platform, or something else. That context lets the LLM decide how shocked, bored, offended, or curious they are when the partner says something impossible.
...
The first sentence is special: it becomes the public roster tagline through `buildVisibleMemberProfile`. Write it as the member's one-sentence self-pitch. It should tell the player who this person is, in that person's voice, with one useful background or personality hook.

Because the app sentence-splits `datingProfile`, do not start with a throwaway sentence and expect the next sentence to carry the hook. If the first sentence is just a name, age, location, job label, or setup for the next sentence, that is all the roster card gets. A clipped question can work only when it is the whole intended public bit, as with Reaver.

Good shape:

- "31, retired from a job that does not have a polite word for it."
- "Rich enough to backstop a Recovery, Captain of the Halid Beg, twelve systems on the manifest, payroll a month behind."
- "My sister made me do this: Hunter, necromancer specialty, 25, on leave I did not request."

Bad shape:

- "Maeve."
- "31, akron oh, journalist (independent)."
- "Tasha, day trader."

Avoid bare name drops, resume fragments, age plus location inventory, or a label with a period. Venus can be an exception when the name itself is the joke and the sentence still performs her status, but do not use that exception for ordinary profile compression.
...
Each character holds their voice fingerprint across the 24-turn default date. The Director applies scenario pressure; the Performer keeps the voice. The Judge is allowed to break voice for clinical scoring; the transcript itself never breaks voice. If two members share patterns (both use Stream of Consciousness, say), differentiate by tics, register, and refused patterns.

### Cupid Intervention (player-typed)

The player's text is the player's text; we do not author it. The wrapper around it is corporate voice and locked: `Cupid suggests: <player text>`. Do not get cute with the wrapper. Cupid interventions target one member at a time. Only the targeted performer receives the nudge in their prompt.
...
Sample lines are rhythm references, not automatic facts about the current date. A performer may introduce a grounded soft detail from a sample, preference, or plain imagination when it gives the partner something to react to. If another member asks about an undefined part of a character's world, the performer may answer with small invented lore that fits the character's existing frame. Once the transcript accepts that detail, it is soft canon for the date.
...
Scenario authoring must stay member-agnostic. Do not name a member in scenario titles, cards, public briefs, events, director instructions, judge rubrics, or early-end triggers. Build reusable hooks instead: Olympus, a bargain clause, an unexplained camera flash, a rescue poster, a formal toast, a quiet corner. The LLM should be the layer that notices the hook because the current speaker is Venus, Vhool, Cassie, Kade, or anyone else whose brief makes the hook matter.
...
- Never editorialize about Cupid, the company, the app, the agency, or matchmaking. The pair leaves on their own terms. Cupid's role belongs in the dashboard chrome, not in the closure note.
