...
This document owns IDC frontend design and theme. `app/app.css` owns the current theme tokens and fonts. UI agents should use those tokens unless existing product docs or code establish a narrower rule. `docs/product/image-style.md` owns image asset style, portrait generation, and the cutout pipeline.
...
Portrait cutouts sit inside Aura surfaces; the dashboard provides the frame and the cutout provides character. See `docs/product/image-style.md` for portrait style and UI placement guidance.
...
- `fieldmote`: cryptids, uncanny household companions, and the talking cat. Earthy slow-drifting dots with a soft glow rising from below. Currently: Calvin (mossy green denied bigfoot), Mr Whiskers (warm tan), Junie (moss and old-woods gold around Otis).
...
- Live Date: the primary room. Setup happens inside this room. Has three states. (1) Planning (no selected date session): a three-step booking flow rendered by `app/components/pre-date-canvas.tsx` inside the Live Date room. The selection order is intentional: date plan first, focus case second, partner third. Step 1 is the date plan, shown as the three drawn `compact` ScenarioCards with Date Book actions. Step 2 is the focus case, shown as the four focus slots (compact MemberCards) with cooldown and ready pills. Step 3 is the partner, shown as a roster grid of eligible standard MemberCards with a "recommended" pill only when one clear booking candidate stands out. Weak, tied, high-pressure, blocked-ask, and boundary-risk options get no recommendation. A sticky bottom dock summarizes all three picks and carries the Begin date CTA. The planning header carries the shift number, shift date status, ready focus count, quits remaining, and the file-shift or next-shift CTA. Closure callouts surface above the booking flow for any ready pair with at least one focused member. Cupid may prefill date, focus, and partner when a conservative recommendation exists. (2) Active date: the live date UI takes the canvas full-bleed. The floating nav cluster hides. (3) Wrap (ended session, before the next book): the date view stays mounted with the FinalReportPanel. The header back button returns to Live Date planning.
...
The floating nav cluster sits at the bottom right of every room. Four round buttons (Live Date, Roster, Date Book, Files) with rose accents on the active button. The cluster hides while a date is live so the live date room owns the canvas; in every other state, including Live Date planning and the post-date wrap, all four buttons are reachable.

### Scenario Card System

`app/components/scenario-card.tsx` is the shared component used in the Date Book deck and library grids and the Live Date planning date-plan step. The scenario's `assets/scenarios/{id}/background.webp` renders as a readable ambient backdrop: barely blurred (`blur-[1px]`/`blur-[2px]`), held at ~45% opacity, seated under a ~42% uniform cream wash plus a faint bottom wash for meter legibility. The image is mood/color, not picture. A glass risk badge sits top-left (LOW/MED/HIGH, color-coded). Content sits directly over the cream wash without a frosted panel, in three sizes:
...
- `compact` (~168px min height, `rounded-[14px]`): used as the three drawn cards in the Live Date planning date-plan step. Carries the title, location, summary, and compact meter row.
- `full` (4:5 aspect, ~260px min height, `rounded-[16px]`): the larger card format retained by the shared component for feature surfaces that need the full card treatment. Carries a small mono LOCATION caption from `publicBrief.location`, the title in `font-display`, a three-line summary in `font-sans` muted, and a three-up stacked meter row showing `Risk · Intim · Chaos` with `LOW`/`MED`/`HIGH` values and matching color dots. Retired entries add a `retired · returns shift N` footnote under the meters.
