...
Each member fixture owns an `apparentHeightInInches` number. Store height in inches only. When height is given in feet and inches during authoring, convert before entering data: `6 ft 4 in` becomes `76`, `5 ft 9 in` becomes `69`.

This field represents the character's visible dating profile height in their chosen or usual presentation. For non-human, shapeshifted, divine, eldritch, synthetic, or otherwise scale-unstable members, use the height of the form Cupid shows on the date surface. Do not use cosmic true scale, mythic title, species scale, or hidden lore as the fixture value.

Height canon may drive comparative standee staging in date surfaces and playground previews. It must not be used to hide asset mistakes. If a character looks too small because the transparent cutout has excess padding, the asset fails the portrait canvas contract above and should be corrected in the source image and cutout pipeline.

Height scaling is a presentation effect. Keep it bounded, bottom anchored, and limited to full-body standees where two characters are compared side by side. Current date standees treat `68` inches as the neutral scale, with a bounded ladder from `0.84` for very short date forms to `1.18` for the tallest date forms. Do not scale avatars, roster thumbnails, compact profile rows, or other cropped identity surfaces by height.
