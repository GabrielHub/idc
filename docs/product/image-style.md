...
Each member fixture owns two height fields, both integers in inches:

- `characterHeightInInches` is the canonical character height. This is the value shown on labels and dossier surfaces, and the value designers reason about ("Calvin is 6'8", Mira is 5'4"). When height is given in feet and inches during authoring, convert before entering data: `6 ft 4 in` becomes `76`, `5 ft 9 in` becomes `69`.
- `standeeRenderHeightInInches` is the input to the date standee scale bucket. It does not have to equal `characterHeightInInches`. Tune it together with the per-member source-scale multiplier in the playground height lineup until the rendered standee reads at the intended height next to its neighbors and the 6 ft guide line.

The character field represents the character's visible dating profile height in their chosen or usual presentation, including shoes, armor, and outerwear that are part of the date-surface silhouette. For non-human, shapeshifted, divine, eldritch, synthetic, or otherwise scale-unstable members, use the height of the form Cupid shows on the date surface. Do not use cosmic true scale, mythic title, species scale, or hidden lore as the fixture value.

`standeeRenderHeightInInches` is a render-tuning knob, not a character fact. It must not be used to hide asset mistakes. If a character looks too small because the transparent cutout has excess padding, the asset fails the portrait canvas contract above and should be corrected in the source image and cutout pipeline.

Generated-height anchor portraits are the calibration set for portrait source scale: Derek Halsey (`76`), Alex Yoon (`73`), Gabriel Tan (`69`), Noah Kim (`69`), and Ryan Doyle (`69`). For anchors, `standeeRenderHeightInInches` is locked to those values because the source portraits are calibrated to them. Normalize anchor sources first so their heights, faces, and proportions read correctly next to each other. After that, scale every non-anchor portrait source against the locked anchor set until faces and proportions read consistently. Treat pose as part of the review, not noise: a lowered head, bent legs, leaning posture, hair volume, armor mass, coat bulk, heels, and shoe soles can all justify a portrait reading shorter, taller, wider, or more compact than a straight standing body. This is an art-direction judgment pass, not a measurement-only pass. Playground source-scale multipliers are audit notes only, final remediation belongs in the source portrait canvas and cutout pipeline. Only after normalized portraits feel right should non-anchor `standeeRenderHeightInInches` values be revised from the lineup.

Height scaling is a presentation effect. Keep it bounded, uniform, bottom anchored, and limited to full-body standees where two characters are compared side by side. Current date standees treat `68` inches as the neutral scale, with a bounded ladder from `0.78` for very short date forms to `1.24` for the tallest date forms. Do not scale avatars, roster thumbnails, compact profile rows, or other cropped identity surfaces by height.

Use the playground Height lineup view to review neutral full-body cutouts side by side after changing height canon or portrait assets.
