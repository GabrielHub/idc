...
- Live Date: the primary room. Setup happens inside this room. Has three states. (1) Planning (no selected date session): a three-step booking flow rendered by `app/components/pre-date-canvas.tsx` inside the Live Date room. The selection order is intentional: date plan first, focus case second, partner third. Step 1 is the date plan, shown as the three drawn `compact` ScenarioCards with Date Book actions. Step 2 is the focus case, shown as the four focus slots (compact MemberCards) with cooldown and ready pills. Step 3 is the partner, shown as a roster grid of eligible standard MemberCards with a "best match" pill on the highest match-fit candidate. A sticky bottom dock summarizes all three picks and carries the Begin date CTA. The planning header carries the shift number, shift date status, ready focus count, quits remaining, and the file-shift or next-shift CTA. Closure callouts surface above the booking flow for any ready pair with at least one focused member. Cupid prefills date, focus, and partner so a one-tap Begin date still works. (2) Active date: the live date UI takes the canvas full-bleed. The floating nav cluster hides. (3) Wrap (ended session, before the next book): the date view stays mounted with the FinalReportPanel. The header back button returns to Live Date planning.
