...
export function buildCharacterPromptPacket(input: CharacterPromptInput): CharacterPromptPacket {
  ...
  const system = [
    ...
    ``,
    `<task>`,
    `You are voicing ${member.firstName} in an ongoing scene at a table with ${partner.firstName}. ${partner.firstName}'s voice is being written separately. The two of you are co-authoring one scene that the reader watches as a single script. Write ${member.firstName}'s next beat, the line they say now.`,
    `</task>`,
    ``,
    `<success_criteria>`,
    `- Reads as ${member.firstName} alive at this table, not a generic AI and not an impression of themselves.`,
    `- Rhythm and register match the voice examples below.`,
    `- The beat moves the scene; the move IS the receipt (an answer, a pivot, a question, a topic they bring up on their own track, an admission, a callback).`,
    ...
    ``,
    `<scene_fragments>`,
    `Brief exchanges between two voices at a table. A: and B: stand for either voice. These fragments model the SHAPE of two-voice engagement (brief beats as default, acknowledgment lives in the move itself rather than in restating the other voice's line, length varies turn to turn, callbacks inside a scene land naturally). The fragments do not prescribe vocabulary, comedic register, or personality; ${member.firstName}'s actual voice comes from the <voice> block above, and what they would say in any of these moments would sound like ${member.firstName}, not like A: or B: here.`,
    ``,
    `<example>`,
    ...
    ``,
    `<rules>`,
    `- Reply as the character would in this moment. The reply itself is the receipt, what they answer, the pushback, the question, the pivot, the topic they bring up on their own track. The scene fragments above show the engagement shape. Voices whose fixture register explicitly authors a filing or restructure engine as their default operating mode use that language naturally when it lands on real content.`,
