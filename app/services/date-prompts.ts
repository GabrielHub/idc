...
export function buildCharacterPromptPacket(input: CharacterPromptInput): CharacterPromptPacket {
  ...
  const system = [
    ...
    ...(greetingSamples.length === 0
      ...
      : [
          `<greeting_examples>`,
          `How you sound when you introduce yourself sitting down with ${partner.firstName}:`,
    ...
    ``,
    `<private>`,
    `Pressure that colors your tone but you do not say out loud:`,
    ...
    `You can see them: ${partner.visualDescription}`,
    `Heights at the table: ${partner.firstName} is ${formatMemberHeightLabel(partner.characterHeightInInches)}, you are ${formatMemberHeightLabel(member.characterHeightInInches)}.`,
...
function buildRecentLineNotices({
  ...
  if (recentSpeakerLines.length > 0) {
    notices.push(
      "Your last lines. The conversation has moved past these:",
  ...
  if (recentPartnerLines.length > 0) {
    notices.push(
      `${partnerFirstName}'s last lines. They have already landed:`,
...
function buildCharacterThreadMessages({
  ...
  if (transcript.length === 0) {
    return [
      {
        role: "user",
        content: `${partner.firstName} sits down across from you.`,
      },
    ];
  }
