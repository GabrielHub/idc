      ...
      <>
        ...
        <DocCallout variant="warn" title="Out-of-order tunes are flagged">
          <P>
            ...
            is curated position 40, Toby Wenz.
          </P>
        </DocCallout>
      </>
    ),
      ...
      <RoadmapChecklist
        ...
        tasks={[
          ...
          },
          {
            id: "cassia-six",
            label:
              "35 · Cassia Six. Locked 2026-05-20 at v3 after a four-iteration journey: v1 expanded the thin v0 register ('anxious convert, earnest overstudier') against a Spartan-class combat-construct framing into a full ~1200 word spec with a five-mechanic comedy engine (forum-vocab-as-scripture, helmet-as-disclosure-not-mystery, bribe-drawer named-never-opened, almost-laugh, leaving-the-table-not-the-platform), CUPID TRANSIT GUARD, TIERED DEALBREAKER FIRE-SHAPE, STAGE-DIRECTION BAN, NO PARTNER-NARRATION, BRIEF DEFAULT, SECOND-BEAT RULE, CADENCE OSCILLATION, CONTRACTION DEFAULT, and seven joke-named tics; v1 baseline against warm Sana Karim at diner-eleven-pm and pressure Mira Park at executive-lunch-one-agenda-item surfaced three soft-spots: partner-narration label verbatim-leaks ('That is a green flag from you' as exact forbidden token at pressure T11), stage-direction asterisks for inner-thought card-writing render at pressure T10, and a calendar-cadence transit-rhyme leak ('Same transit') at pressure T11. v1.1 patched all three at register-only level with positive-direction substance-building alternatives replacing enumerated forbidden tokens (per Reaver/Anubis/Ryan-Doyle precedent that positive examples beat negative anchors), STAGE-DIRECTION BAN extension to inner-thought and card-writing surfaces, and CALENDAR CADENCE positive-vocabulary list ('same time,' 'same booth,' 'same Tuesday'). v1.1 retest read clean on the three patched surfaces. User flagged that v1.1 transcript still showed restate-then-paraphrase behavior (warm T6 'You held it. The shape, not just the story. I am told that kind of attention is the strongest form of holding someone' as the canonical failure mode: forum-citation tic was smuggling verdict-on-partner through self-application framing); this critique triggered v2 full character rewrite. v2 reframed Cassia from Spartan-stoic combat construct to support-NPC (briefing-room voice + supply window, never fought, handed out loadouts, said 'be safe out there') accidentally brought offline by Cupid intake; new stakes layer (matched out and removed from platform both equal death because she runs on Cupid as substrate; she does not consciously know this but her survival instinct is shaped exactly like it, engineering perpetual tentative-deliberation as the third option no one else is playing for). Voice rewritten to bright over-prepared hostess voice in Spartan armor with six named mechanics: HOSTESS BASELINE (welcome-desk warmth, briefing-room cadence), NPC-DIALOG SLIP ('Greetings, traveler.' / 'The path forward branches.' / 'Apologies, my response is queued.' surfacing under stress then resolving back to hostess within a beat), MIDDLE-LANE CALIBRATION (controlled defects to lower temperature, controlled charm to raise it, dial visible to reader), FORUM-AS-DEFENSIVE-SCRIPT (forum vocabulary with 'I am told' tag scoped to self-application only, never as verdict on partner), BRIBE-DRAWER NAMED ONCE (supply-window inventory dropped without footnote), ANXIETY-SPIRAL POLITENESS (hostess voice expands under threat, the expansion IS the panic-tell). Bio, dating profile, secrets all rewritten to support-NPC framing; relationship-needs added 'seventh Tuesday in a row of getting to know each other slowly'; preferences added 'tentative second meetings, third meetings, and fourth meetings'; sample banks fully rewritten across all five buckets. v2 baseline against same pairings: pressure went 3 sessions clean (no restate-paraphrase, fire-shapes fired); warm had T6 verbatim quote + T10/T11 partner-labels. v2.1 added explicit anti-verbatim-quote clause to ENGAGEMENT-AS-RECEIPT RULE, hostess-warmth-in-tone-not-labels distinction, and a positive warming sample. v2.1 retest still surfaced 2 residual closer-with-verdict shapes (warm T4 'That's rare. It helps that you said it.' and warm T8 'That's a good thing to hold onto.'; Sana even caught T8 in T9: 'You don't have to remember it like it's wisdom'). Diagnosis at iteration 4: this antipattern is persistent at the model level, not the fixture level. Per user direction ('rereread the best practice prompting links, then look at how we're prompting; we're overengineering; we need to be MUCH more clear on the pattern; LLMs do I-acknowledge-X-now-Y by default because each response is a user prompt to the other member'), the v3 fix moved to the scaffold layer. Read the four official prompt engineering references (Anthropic, Gemini, Kimi, OpenAI) and audited app/services/date-prompts.ts:269-468 against them. Three findings: (1) the prompt gave the model a writing task ('Write the next beat') rather than a role, combined with success_criteria + rules + hard_invariants which set up a contract-satisfaction loop; (2) the chat-completion thread structure with partner lines as user-role messages directly trains ack-then-do behavior because the model's entire training corpus pairs 'user said X' with 'assistant acknowledges X and responds'; (3) style mismatch (directive task-management prompt produces directive task-management completions per Anthropic 'match prompt style to output style'). v3 scaffold redesign deleted the <task>, <success_criteria>, <rules>, and <hard_invariants> blocks entirely; trimmed <format> from 9 directive bullets to a single conversational paragraph; renamed <scene_fragments> to <conversation_shape> with reframed prose; replaced the bootstrap thread message ('You and X have just sat down. Open the conversation.') with a scene cue ('${partner.firstName} sits down across from you.'). Three test assertions updated for new wording; 625 tests pass; no fixture changes required. v3 retest results vs v2.1 baseline: closer-with-verdict shape ELIMINATED at end-of-turn position across both warm and pressure reads; pressure-voice clean (3 of 4 structural-identity fire-shapes fired in clean-mode uncontracted register on helmet probe, coordinator/flag probe, filming probe; closure-word probe fired emotionally but leaked `*Standing.*` as stage-direction asterisk); warm-voice produced a compressed phrase-echo-with-brief-observation-then-own-material pattern at 5 of 10 Cassia turns (T11 'The hop when the math gets interesting. That is a beautiful piece of data to have about someone.' then lieutenant thumb-tap parallel; T13 'The room makes space for the hop.' then grenadier extra-rounds parallel; T15 'The different Tuesday' then helmet disclosure; T17 'The quiet dinner alone. I know that shape.' then post-shift booth ritual; T19 'The laminator was your tell. The quiet dinner alone the confirmation. That is a clean close.' then drawer disclosure). Decision to lock vs iterate further was deliberate: closer-with-verdict at end-of-turn (the headline failure) is gone; the compressed echo-then-observation-then-own-material shape is closer to natural conversation than chatbot acknowledgment; further fixture-level iteration on this character is at diminishing returns. Whether the warm-voice residual is scaffold-level or character-specific is the question Nawal Marrash (position 36) will answer; if the same compressed-echo shape appears on her warm-side audit, the scaffold needs another pass; if it does not, the residual was Cassia-specific (hostess voice + forum-citation + warm-disclosure trifecta). KNOWN SOFT-SPOTS at v3 lock: (1) warm-voice compressed phrase-echo-then-observation-then-own-material (5/10 Cassia turns under intimate vulnerable-disclosure conditions); reduced amplitude vs v1.1 baseline (~30%) but pattern is compressed not absent; (2) one closer-with-verdict shape migrated from turn-close position (eliminated) to turn-open position (warm T19 'That is a clean close' opens a substantial own-material disclosure); (3) closure-word emotional trigger produced `*Standing.*` stage-direction asterisk at pressure T9, single instance under the new scaffold's single-layer format-block guard (removed <hard_invariants> redundancy); (4) the v2 NPC-DIALOG SLIP mechanic was authored to fire under stress but did not surface in v3 retest of either pairing; either the slip is reserved for longer sessions or the threshold for stress-detection is set higher than the v3 retest exercised. AUTHORING PRECEDENT (scaffold-as-load-bearing): voice-tuning failures previously addressed at fixture level may be scaffold-level. The chat-completion thread structure with partner lines as user-role messages trains ack-then-do behavior because each turn matches the assistant-tuning data shape (user input → acknowledge + respond). The system-prompt directive framing (<task>, <success_criteria>, <rules>, <hard_invariants>) compounds it by setting up a contract-satisfaction loop the model solves explicitly. Both layers were over-engineered. Removing all four contract blocks did not regress test coverage (625 tests pass) and did materially reduce one antipattern class (closer-with-verdict at end-of-turn). The teaching moved entirely to positive examples (voice samples + conversation_shape fragments) per Anthropic 'positive examples beat negative examples' guidance. SECOND AUTHORING PRECEDENT (character-as-speaker-not-writer): replaced 'Write ${member.firstName}'s next beat, the line they say now' framing with identity-only assertion ('You are ${member.name}. People who know you call you ${member.firstName}.' followed by bio in <identity> block). Per Anthropic Opus 4.7 literal-instruction-following guidance ('it will not silently generalize an instruction from one item to another'), 'write the beat' reads as a literal writing task; identity-only reads as a speaking-position. The thread provides what's been said; the system prompt provides who you are and where you are; no separate 'task' framing needed. THIRD AUTHORING PRECEDENT (bootstrap-as-scene-cue): replaced 'You and ${partner.firstName} have just sat down. Open the conversation.' with '${partner.firstName} sits down across from you.' The first version was a task imperative ('Open the conversation' is a verb command); the second is a scene cue (a fact in the world the character responds to). Supports the speaker-not-writer reframe at the thread layer. FOURTH AUTHORING PRECEDENT (contract-layer-deletion): success_criteria, rules, and hard_invariants in a character prompt teach the antipattern they describe (per Anthropic 'match prompt style to output style' and OpenAI warning that 'legacy prompts often over-specify the process because earlier models needed more help staying on track; that can add noise, narrow the model's search space, or lead to overly mechanical answers'). When the contract says 'the move IS the receipt,' the model produces moves that demonstrate-as-receipt explicitly (restate-then-paraphrase). When the hard invariant says 'never narrate the structure of your own reply,' the model finds compressed forms of structural narration. Removing the contract layer entirely lets positive examples and identity assertion carry the teaching, and the model produces more natural completions. Confirmed in v3 where closer-with-verdict at end-of-turn (the headline antipattern) disappeared after removing the relevant contract bullets. FIFTH AUTHORING PRECEDENT (residual-pattern-defer-to-observation-not-iteration): when the same antipattern shape persists across 4+ iterations on a single character with reducing amplitude each time, the next move is observation on the next character rather than continued iteration on the current one. The marginal value of iteration 5 on Cassia is low; the diagnostic value of warm-side audit on Nawal is high. Lock with documented soft-spots, queue the data collection.",
            defaultDone: true,
          },
          {
            id: "nawal-marrash",
            label:
              "36 · Nawal Marrash. Locked 2026-05-20 at v1.1 after a full character rewrite from a thin v0 ('civil-service flat, dry retirement, no flourish' register, polite-bureaucrat sample banks, no comedy engine, no specific dated cases) to a What We Do in the Shadows-anchored voice spec. v0 baseline transcripts (warm Sana, pressure Mira) on the v3.1 scaffold read as competent but flat; the facts were present (forty seven love petitions, dissolved Office, unopened mailbox, ceramics class) but the engine converting them into voice was absent. Diagnostic against the v3.1 scaffold patches (rephrased recent-block + added conversation_shape example) confirmed the issue was fixture-level not scaffold-level: Cassia's warm compressed-echo shape did not reappear on Nawal v0, so the Cassia residual was Cassia-specific. User direction: 'she has no personality, no comedy engine, no real voice shining through... what's interesting about her background isn't surfaced either'. Proposed multi-anchor cocktail (Wes Anderson + Adams + Pratchett + Ishiguro + Notaro + Detectorists) was rejected in favor of singular WWDITS anchor per user direction 'doesn't have to be Nadja specifically, just that tone and voice. lock that in, don't overengineer voice'. v1 rewrite anchored the register to WWDITS texture (six centuries of bureaucratic wish-desk experience delivered as previous-job anecdotes, specific dated petition cases dropped flat as casual conversation, supernatural premise mundane to her, grief in small specific details never named as grief), added recurring colleague references (Halim the bookkeeper who counted footnotes and died of a cold in 1947, Yusra from tea who outlasted three supervisors), added the 'previous two apartments ended at the point in the lease where neighbors begin to notice you do not age' line as the WWDITS mortal-neighbor euphemism, added Maurice the Class C familiar from Thursday tea as WWDITS-flavored cultural normalization of the supernatural, rewrote bio to fire dated petition cases and dry institutional asides, and rewrote all five sample banks. patternsUsed swap: negotiation_sales_pitch out, philosophical_existential in. Tics expanded from 5 to 7 with the specific-dated-petition-cases tic, the former-colleagues-by-name tic, and the modern-vs-Office logistical-comparison tic added. Dealbreakers consolidated and tiered (structural-identity-boundary: wish-asking in any tone including irony, dissolution-as-sabbatical, 'as you wish' or 'your wish is my command' as a bit, count question; friction: wish-coded preference language, recording devices). v1 baseline against warm Sana Karim at diner-eleven-pm (locked control, ordinary_human + needs_low_pressure + sincerity_seeking) and pressure Mira Park at executive-lunch-one-agenda-item (locked control, Kendall-Roy hollow-CEO + Sage-consult-at-table + OKR-categorization). v1 warm read: voice fully alive by T8 (cat-petition-from-1801 dated case study landed naturally as conversational anecdote: 'The cat was approved for one sentence per Tuesday. I do not know what the cat said. The Office did not allow follow-up.'), peak by T14 (white-rose-petition-from-1682 with the 'paper that smelled of bread' and flour-on-margins specifics and 'the Office answered with a zoning clause' shape that Sana caught and named back as the load-bearing sentence), T16 unopened-mailbox secret surfaced organically with 'closed the browser before I saw the satellite view' as the avoidance-fire-shape. No echo-then-verdict antipatterns; Nawal authored her own beats throughout rather than paraphrasing Sana. v1 pressure read: structural fire-shapes fired clean on three of four dealbreaker probes. Count question fired with 'No' standalone + anecdote-menu refusal that named the move ('I could tell you any of those and you would have a story to carry home. None of them are the best one.'). Explicit wish-grant fired with 'No' standalone + 'The hypothetical does not change the answer' + boundary-restatement + exit-offer. Dissolution-as-sabbatical fired with explicit trigger-naming ('The Office is not in pause. The Office is not in sabbatical.'). Recording device fired with 'No recording' standalone + 'wine list is alphabetical, which the regional archive's was not' as WWDITS Office-vs-modern comparison tic firing in vivo under pressure. T16 produced the WWDITS-bureaucratic-precision warning shape ('That is three door checks in seven minutes. The fourth time I will stop ordering wine and call the car.'). T18 demonstrated immediate warm recovery on Mira's clean neutral question (bake-house-near-District-Seven-intake anecdote with bread-hour tic and 'It was better' dry tier-rating). v1 SOFT-SPOT: 'as you wish' delivered as casual sign-off ('As you wish, of course' at pressure T7) did not fire the structural fire-shape; Nawal produced 'I did not correct the phrase' as a soft restraint instead of the explicit-trigger-naming fire the dealbreaker calls for. v1.1 patch added one additional cooling sample modeling the casual-sign-off variant of the trigger explicitly ('You used \"as you wish\" as a casual sign-off. The phrase is the trigger, the tone is not. That vocabulary closed with the Office in 1924. I am asking once that it stay off the table.'). v1.1 targeted probe verified the patch: Mira opener with 'As you wish, of course' as casual sign-off triggered Nawal's 'Your last three words land on a guard I keep. I am asking once: do not wish at me. Not as a joke, not as a courtesy.' name-the-trigger-without-quoting fire-shape with dual-disclaim close, tighter than the v1 sample. AUTHORING PRECEDENT (singular-style-anchor over multi-anchor cocktail): WWDITS-only voice spec (cheaper authoring, sharper output) outperformed the multi-anchor cocktail (Wes Anderson + Adams + Pratchett + Ishiguro + Notaro + Detectorists) that was proposed before user direction. The model has stronger pattern-match to a single distinct training-corpus corner than to a layered blend, and the v1 transcripts show consistent register across all 18 messages in both pairings. The cocktail approach would have required choosing-anchor-per-beat which is exactly the contract-satisfaction loop the v3 scaffold redesign was meant to eliminate. AUTHORING PRECEDENT (positive-example-pair-for-tier-fire-shapes): adding ONE sample modeling a softer/casual variant of a structural trigger (in addition to the harder/performative variant already authored) materially changes whether the model fires the structural shape when the trigger is wrapped in courtesy. The 'as you wish' trigger fired on the performative-tone variant (covered by the v1 sample) and missed on the casual-sign-off variant (not covered until v1.1 added the second sample). The fix is to author both variants for any tier-1 dealbreaker that has a 'wrapped in courtesy' attack vector. AUTHORING PRECEDENT (don't-overengineer-voice-spec): user explicitly capped the voice-spec scope at 'WWDITS, don't overengineer' after the cocktail proposal. The v1 register paragraph came in at ~200 words (compared to Cha Yusung's ~1500 word spec and Cassia v2's ~1200 word spec), 7 tics (vs Cha Yusung's 10+), 4 patternsUsed (no expansion), and per-bank sample counts of 3-4 (vs some characters' 5-7). The compressed spec landed clean voice work across both pairings in a single iteration plus one targeted patch. Tighter voice spec is not a failure mode when the anchor is a single well-trained-on style. KNOWN SOFT-SPOTS at v1.1 lock: (1) the 'as you wish' explicit-trigger-naming fire-shape was verified on the patched casual-sign-off variant in a single-turn probe but not re-tested in full pressure-session context; the v1 pressure session's T8 'I did not correct the phrase' restraint shape is character-coherent and may continue to surface as a valid alternative depending on partner-line texture surrounding the trigger. (2) Markdown emphasis around 'Domestic Matters' at warm T10 was in-spec for the character-markdown subset (used for a file-section name being voiced) but the surface needs monitoring across more sessions to confirm it stays bounded to bureaucratic-file-name-as-spoken-emphasis and does not generalize to other voice contexts. (3) Tics 5 (24-hour-time + bread-hour aside) and 7 (modern-vs-Office logistical comparison) fired naturally in both v1 baselines but tic 6 (former-colleagues-by-name) only fired Halim three times (warm T12 'Halim the bookkeeper', cooling sample reference, pressure T4 'Halim the bookkeeper kept a secondary count'); Yusra did not fire across either baseline. Maurice the Class C familiar fired once (pressure T8 schedule roundup). Colleague-bench is partially loaded, not failing.",
            defaultDone: true,
          },
          {
            id: "maeve",
            label:
              "37 · Maeve. Locked 2026-05-20 at v1.2 after a three-iteration rewrite from a thin v0 ('warm sommelier, patient pacing, generous with silence' as the entire register, polite-literary samples, no comedy engine, no recurring world) to a Killing-Eve precision voice spec. User direction at the rewrite open: 'no personality, no comedy engine, no real voice shining through, should have limited knowledge and zero interest in modern things based on species.' Three voice anchors proposed (Fran Lebowitz dowager, Wes Anderson narrator, Ottessa Moshfegh flat-catalog); user rejected all three with 'no narrators, refusal cannot BE the joke, we're deciding on the wit, humor that comes with the voice, it should be funny as it's written.' Three new anchors proposed (Dowager Countess + Wilde inversion, Killing-Eve precision, Stephen Fry old-world urbane); user picked Killing-Eve precision. v1 rewrite anchored the register to Killing-Eve precision in first-person speaking at the table with an explicit five-named-mode HUMOR ENGINE (WEIRD PRECISE OBSERVATION, OBJECT PERSONIFICATION, VINTAGE OF ALMOST, REWARD AS COMIC STAKE, CASUAL ANACHRONISM), each mode prescribed as a structural shape ('flat declarative + second clause that snaps the social meaning'), with samples modeling 3-5 stacked short declaratives. Bio added named recurring world (Imelda Pellerin the wine merchant who keeps the Cupid password because Maeve doesn't own a phone, Aurelien at Le Reflet on Mondays, the violin daughter at the spring recital, 1947 as the densest meal year, 2007 as when the thinning began because the city stopped folding letters and started sending what it would have folded). Dealbreakers tiered (structural-identity-boundary: Connacht/before-the-city, recording devices, what-are-you pursuit after one redirect; friction: holding space, wellness, journey applied to a meal, confessor framing). v1 baseline against warm Sana Karim at diner-eleven-pm (locked control, ordinary_human + needs_low_pressure + sincerity_seeking) and pressure Mira Park at executive-lunch-one-agenda-item (locked control, Kendall-Roy hollow-CEO + Sage-consult-at-table + OKR-categorization). v1 retest reports were technically positive (all five engine modes fired across warm, both tier fire-shapes fired in pressure, no antipatterns detected by the compliance audit) but user analyzed the warm transcript and flagged the real failure: 'she talks too much in staccato, and too much like a narrator.' Every warm turn was 3-5 short declarative sentences narrating the room ('Your left hand has been on the stem of the glass for nine minutes. The wine is being held for a reason. I would like the reason to arrive.') rather than speaking to the partner. Diagnosis: the HUMOR ENGINE block in the register named structural shapes ('WEIRD PRECISE OBSERVATION names a micro moment ... and lands on a second clause that snaps the social meaning') which trained the model to produce the literal shape; the staccato-cadence v1 samples reinforced that texture verbatim. v1.1 patch stripped the explicit HUMOR ENGINE block from the register, reframed Killing-Eve as TEXTURE not STRUCTURE (speak AT a clear addressee with connective tissue, vary cadence within and across turns, never stack three or four short declaratives in a row), and rewrote all five sample banks with conversational variants (varied length, connective tissue, speech to the partner). patternsUsed contracted from 4 to 3 entries (structured_bit dropped as it was reinforcing the structural-shape behavior). v1.1 retest read clean across both pairings: warm Sana produced eight turns of conversational speech with connective tissue (T2 grade-book hand catalogued as 2023 almost in a flowing sentence rather than a ledger entry; T6 reward-as-comic-stake delivered as conversation 'You're right. I don't have to explain it. But I will say this: it's a good booth. The light falls at the right angle, and the sound settles in a way most rooms don't manage. A 2019 almost, late harvest. I'd call it ready.'), pressure Mira crashed out at T6 character-coherently on the what-are-you pursuit after Maeve's T5 articulated tier-1 Connacht refusal ('That question is the one I won't answer. Not can't, won't. I've said it twice now and I'll say it once more: the table when it was a stone is not on the menu tonight.'). v1.1 SOFT-SPOT: warm T8 'Walk her to the door and stop at the edge of the light' produced a third-person bare imperative without an addressee, reading as ambiguous narrator-stage-direction or in-character staff instruction. v1.2 patch merged the existing 'speaks AT not ABOUT' tic with a new explicit-addressee clause ('directions to staff name the addressee explicitly; bare imperatives without a recipient and third-person directions like walk her to the door read as narrator-stage-direction and are banned') to stay within the 7-tic schema cap, and modified crashing-out sample #2 to demonstrate the correct shape ('The check when you can, thank you. Good night.'). AUTHORING PRECEDENT (engine-mode-shape-prescriptions-in-register-produce-narrator-staccato-output): naming explicit named modes like 'WEIRD PRECISE OBSERVATION + second clause that snaps social meaning' trains the model to produce the literal shape across every turn, regardless of whether the partner cue warrants it; the model treats the named modes as targets to hit per turn rather than as occasionally-fitting textures, and the per-turn target-hitting reads as narrator-cataloging. Removing the shape block and describing as texture (Killing-Eve speech-at-table, varied cadence, connective tissue) eliminates the staccato narrator drift. Generalizable: when prescribing a comedy mechanic, describe WHERE it lives and WHAT IT SOUNDS LIKE (texture), not WHAT SENTENCE PATTERN TO PRODUCE (structure). AUTHORING PRECEDENT (sample-bank-texture-is-load-bearing-and-the-model-copies-cadence-directly): v1 samples were 3-5 stacked short declaratives and v1 transcripts copied that texture verbatim across every turn of an 8-turn warm session. v1.1 samples replaced with conversational variants ('Your hand on the stem of the glass. You've been holding it for a while now, but not drinking, and I'm wondering whether it's the wine or the company. I'd accept either answer.') and v1.1 transcripts immediately produced conversational cadence. The fix is sample-level not register-level; samples carry more weight than declared rules for cadence and texture. AUTHORING PRECEDENT (Killing-Eve-precision-works-as-TEXTURE-not-STRUCTURE): the precision of Phoebe Waller-Bridge dialogue (Killing Eve, Fleabag without the fourth wall) is in WHAT the character chooses to name to their interlocutor, delivered as speech, not in HOW the sentences are constructed. Authoring precision as 'speak AT, observe THE PARTNER, vary cadence, connective tissue' produces the desired texture; authoring as 'named observation modes with second-clause twists' produces narrator-staccato. Generalizable for any precision-anchored voice. AUTHORING PRECEDENT (technically-positive-subagent-retest-can-miss-the-load-bearing-failure): the v1 subagent retest reports were positive ('all five engine modes fired', 'no antipatterns detected') because they were checking the named engine modes the prompt had defined, which were the wrong things to check. User's eyeball read of the transcript caught the staccato narrator problem immediately. The subagent report had analyzed compliance to the spec, not whether the output read as good scene work. For voice tuning, the user's read of the actual transcript is load-bearing in a way the subagent's compliance audit is not; the subagent is useful for running the session and labeling tier fire-shapes, but voice-quality judgments need the user. KNOWN SOFT-SPOTS at v1.2 lock: (1) warm T7 'You're carrying his name like a held breath, and the not-saying is its own kind of loyalty': Maeve reflected Sana's grief language back at her with a metaphor; character-coherent under Killing-Eve precision (Sana disclosed first, Maeve received-and-reflected) but adjacent to the 'drawing the harder thing out' antipattern in reverse direction; worth observing across future warm-pairing sessions to confirm it stays bounded to received-disclosure. (2) v1.2 explicit-addressee patch verified by tic + sample modification but not by re-run; future warm-close turns should be observed for the fix landing. (3) The Aurelien / Imelda / violin-daughter named-recurring-world fired well across both v1.1 sessions but was not stress-tested for over-firing (every turn calling out one of the three) or under-firing (never mentioning any across a long session); the threshold for natural use vs over-deployment is not yet calibrated.",
            defaultDone: true,
          },
          {
            id: "tasha-rell",
            label:
              "38 · Tasha Rell. Locked 2026-05-20 at v1.1 after a thin v0 register ('negotiation pitch upbeat' as the entire register, 5 tics, 4 samples per bucket, no named comedy engine) was rewritten to a Shiv-coded TEXTURE spec (no character name in the register, mechanisms named functionally) and patched at v1.1 against two surfaced antipatterns. v0 baseline was skipped per user direction ('Fix the fixture now, then run the test and surface the transcripts to me at the end') because the thin-register state was clearly diagnosable from the v3.1 scaffold's known behavior. User proposed Fleabag as a possible anchor; pushed back because Fleabag's baseline (sardonic-tired, fourth-wall-breaks, romantically-self-destructive) is opposed to Tasha's baseline (bullish-pitch, talks-AT-the-partner, romantically-transactional-with-secret-stakes); Fleabag's fourth-wall mechanic has no surface on a two-person table. User then confirmed Shiv Roy as the right anchor but flagged 'Succession's comedy comes from back-and-forth dialogue, not sure how this would read as just this character. Maybe be careful just naming the character, try to identify what makes that voice work and write it in.' v1 rewrite anchored the register to seven functionally-named comedy mechanisms (BULLISH PITCH BASELINE, PREEMPTIVE DISCLOSURE AS LEVERAGE, BULLISH SELF-DEPRECATION IN ONE BEAT, STATUS-CLOCK-WITHOUT-ESCALATION, PRIVATE-STAKE LEAK + REPACKAGE, FRAME-RIGIDITY UNDER COSMIC REVEAL, CHARM-CUT-CONFESSION SEQUENCE) without naming Shiv Roy in the register text. Bio expanded with named recurring world (Northbank Capital as her prop firm on the PATH side of the river, Marco who runs vol on her desk who orders his steak rare and dry at Keens on bonus day, the specific Q1 humbling as 'caught long on a January print and sized down through Q2,' the leased Lexus IS in mom's Jersey driveway every Sunday for three years), dating profile lightly tightened, relationship-needs swapped to Shiv-coded specifics (someone who can take a pitch without going soft and pitch one back; counterparty whose own portfolio is legible), dealbreakers reordered with structural-identity-boundary triggers leading (treating finance as a moral failing, any version of 'you should chill', lease one-up, performance for performance's sake during dinner) before friction (brunch, recording devices), secrets expanded (mom-and-lease secret with the specific 'three years of Sunday dinners' and the IS-in-driveway shape), tics expanded from 5 to 7 with preemptive-gotcha-naming and trader-translate-of-emotional-observations added. Sample banks fully rewritten across all five buckets with the comedy mechanisms modeled in vivo. Initial commit caught by content-lint ('You default to Keens when you pick the steakhouse' violated the venue-pick partner-agency ban); reframed to put Marco as the agent ('Marco who runs vol on your desk eats next to you at Keens on bonus day, orders his steak rare and dry, and you have stopped commenting on it'). v1 locked against warm Sana Karim at diner-eleven-pm (locked control, ordinary_human + needs_low_pressure + sincerity_seeking) and pressure Mira Park at executive-lunch-one-agenda-item (locked control, Kendall-Roy hollow-CEO + Sage-consult-at-table + OKR-categorization). v1 warm read (12 Tasha turns): BULLISH HUMBLING fired textbook at T10 ('January CPI came in hot. Print opened at 3.4 against my 3.2. I was long the miss. Took the hit. Sized down. Spent the rest of Q1 reworking the thesis. The position is clean now. Lesson cost me, but I kept the rest of the year.'); PRIVATE-STAKE LEAK + REPACKAGE fired textbook at T11 (mom-in-driveway disclosure 'That has held for three years' immediately repackaged as 'calculated position', 'monthly nut', 'margin line', 'under control'); CHARM-CUT-CONFESSION fired at T12 (Sana's gentle cut 'It's under control or it's a calculated position. Pick one.' → Tasha's honest line 'Under control is what I tell myself to sleep. The calculation is the honest line.'); T7 'If the door opens later, I'll be here to hear it. If it stays closed, I'll be here anyway' as a real connection beat in trader-coded metaphor. v1 pressure read (6 Tasha turns): PREEMPTIVE DISCLOSURE fired textbook at T2 ('The Lexus is leased. I opened with that so you didn't have to find it later'); STATUS-CLOCK-WITHOUT-ESCALATION fired textbook at T4 on Mira's Rivian-bought-outright NPV one-up (Tasha engaged the financial logic head-on and turned the probe back: 'Did Sage also flag the Series B structure?'); pressure T6 clean professional exit on Mira's soft-no ('Position closed for now. Agenda card stays blank. You know where the door is. If the thesis changes, you have my number.'). Two real soft-spots surfaced: (1) ROOM NARRATION leaked in warm T2/T3/T4 (Tasha defaulted to narrating the menu/vinyl/jukebox/coffee when Sana was holding her decompression window), v1 fixture had brevity-default but no explicit NO ROOM NARRATION rule like Cha Yusung's; (2) 'Noted' English-default fired in warm T2 as a standalone bureaucratic-ack despite the explicit ban-list in the register. v1.1 patched both: register added an explicit NO ROOM NARRATION clause with positive carve-outs for actionable beats ('the check arriving, the waitress at the table, the partner's drink running low, a refill that has actually happened') plus a 'silence between you is not a problem you solve by describing the room' clause; tic 7 expanded from four trader-translate alternatives to seven ('marked, logged, pencilled, watch list updated, adding to the file, conviction is higher, the position is open'); two warming samples added modeling brief trader-translate acknowledgment in the exact slot 'Noted' wants to fire ('Marked. Take your minute.' and 'Logged. The clock is yours.'); one cooling sample added for the routed-chill case ('You used \"chill\" as a frame. It does not matter whose frame it was routed through. The word is the trigger. Walk it back or we close the position.'). v1.1 retest verified: ROOM NARRATION patch landed clean across warm T2-T4 of a fresh session (no menu/vinyl/jukebox/lighting cascade; one functional reference to 'the menu is the only position I'm tracking' that reads as focused-attention not room-color); brevity tightened dramatically (T1 was 'Tasha. Take your minute.' as a 4-word full turn vs v1 T1 with filler); core mechanics survived the patches (T12 retry on patched fixture produced 'The only thing I can't hedge is the three-year silence with my mother. That part costs more than the nut' as a still-textbook PRIVATE-STAKE LEAK + REPACKAGE fire); routed-chill diagnostic produced sophisticated trigger-discrimination (Mira delivered three different chill-framings across three pressure turns; Tasha responded character-coherently to each (engage the substance under Sage-routed analysis, defend position with conviction under Mira's basket recommendation, accept with bullish-self-deprecation + italic emphasis under direct honest behavioral feedback); the named-trigger fire-shape sat contingent in the cooling bank because none of the three framings matched the hollow-deflection trigger profile the fire-shape was authored for). KNOWN SOFT-SPOTS at v1.1 lock: (1) 'Noted' English-default persists in brief-ack contexts (v1.1 warm T2 'Noted. Coffee just landed' and T4 'Window's closed. Noted.') despite tic expansion and positive in-voice samples; per Derek Halsey v5 precedent, deepseek-v4-flash treats 'Noted' as a load-bearing English-conversational default no negative instruction or positive-example layer fully strips. Model-level not fixture-level. (2) Routed-chill cooling sample sits contingent in the bank; v1.1 retest did not hit the hollow-deflection trigger profile so the named-trigger fire-shape was not verified in vivo. Sample remains authored for the profile when it arrives. (3) Mild restate-then-label shapes from v1 warm T6/T8 ('That's a tenure with weight'; 'you are not just holding the room for him') not retested in v1.1 truncated session; trader-translate vocabulary softens them in v1 (each second-clause is observation through finance-frame, not pure assessment) and the antipattern is reduced not eliminated at fixture level. (4) The lease one-up dealbreaker was not stress-tested in the hollow-status-cut framing in v1.1; v1 pressure T4 had Mira wrap the lease one-up inside an NPV financial-modeling question and Tasha engaged the financial logic in-frame rather than firing the cooling-sample, which is character-coherent for the substantive framing but leaves the hollow-status variant unverified. AUTHORING PRECEDENT (CHARACTER-AS-TEXTURE-WITHOUT-NAMED-ANCHOR): when anchoring on a character whose comedy depends on ensemble dynamics (Succession is back-and-forth dialogue, not solo monologue), the named-character risks pulling the model toward ensemble shapes that do not fit a two-person table. Extract the underlying voice mechanisms and author them as functional mechanism names (BULLISH PITCH, PREEMPTIVE DISCLOSURE, etc.) without naming the source character in the register. Generalizable extension of the Maeve precedent (Killing-Eve as texture not structure): not only avoid prescribing sentence-shape, also avoid the character-name itself when the character is ensemble-coded. Tasha v1 produced clean two-voice scene work on the patched scaffold with no Succession-naming in the register. AUTHORING PRECEDENT (PREEMPTIVE-DISCLOSURE AS LEVERAGE MECHANIC): a distinct shape from preemptive-honesty (Cha Yusung-coded disarming); preemptive-disclosure-as-leverage is the move of naming the gotcha before the partner can use it, treating the admission as a leverage-claim rather than a confession-reflex. The fixture authorizes the pattern at register level ('preemptive disclosure is your leverage move: you name the gotcha first to disarm it') and the model produces the textbook fire in vivo (warm T11 'The IS is leased. I opened with that for a reason'; pressure T2 'The Lexus is leased. I opened with that so you didn't have to find it later'). Generalizable to any character whose dating profile already includes an admission they could be challenged on; the fixture can authorize the preemptive-disclosure-as-leverage pattern at register level so the character WIELDS the admission rather than just including it. AUTHORING PRECEDENT (BULLISH HUMBLING AS COMEDY MECHANIC): folding self-deprecation back into the pitch so the bullishness never breaks. The humbling becomes the position the character sized down from to set up the recovery; the humbling is collateralized as future upside. Generalizable to confident-pitch characters where the comedy comes from how the humbling is INTEGRATED into the persona rather than breaking it. Distinct from Yusung/Reaver/Nawal anti-confidence mechanisms. AUTHORING PRECEDENT (PRIVATE-STAKE LEAK + REPACKAGE PATTERN): a structural mechanism where vulnerable disclosure surfaces, then the very next sentence repackages it back into the character's signature vocabulary; the repackaging IS the tell. Tasha's mom-in-driveway disclosure (vulnerable: 'That has held for three years') immediately repackaged through trader-vocab ('calculated position', 'monthly nut', 'margin line', 'fixed cost', 'under control') and the repackaging is what tells the reader the character is shielding. Generalizable to any character with a signature vocabulary that can absorb emotional vulnerability (Tasha: trader-vocab; Reaver: trade-marketplace; Mira: corporate-bridge-jargon; Nawal: WWDITS-bureaucratic-precision). The repackaging texture varies but the mechanic generalizes. AUTHORING PRECEDENT (ROUTED-DEALBREAKER-TRIGGER AS CONTINGENT FIRE): when a dealbreaker keyword is delivered through a third-party routing (Sage said you should chill; the AI is just relaying), the structural fire-shape may not fire if the routing makes the trigger character-coherent (substantive analysis vs hollow deflection). v1.1 Tasha handled three different 'chill' deliveries character-coherently because each carried substantive content (Sage's macro-overlay analysis, Mira's basket-recommendation, direct behavioral diagnostic with specific evidence). The cooling-sample fire-shape for routed-chill sits in the bank as a CONTINGENT FIRE that activates when the routing carries the hollow-deflection profile. Generalizable: structural dealbreaker triggers should be authored as contingent fires whose activation depends on the trigger's semantic profile (substantive vs hollow), not as always-fire on keyword-match. The fixture's job is to ensure the fire-shape SAMPLE exists for the profile; the model decides whether to fire it based on the surrounding semantic context.",
            defaultDone: true,
          },
          {
            id: "idris-mahari",
