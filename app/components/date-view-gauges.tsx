...
import { scoreWidthClass, Tooltip } from "./dashboard-atoms";
...
function HealthGauge({ value }: { value: number }) {
  ...
  return (
    <Tooltip
      message="Overall room health. Conflict lowers it; chemistry raises it."
...
function TurnGauge({ current, total }: { current: number; total: number }) {
  ...
  return (
    <Tooltip
      message="Date length. Cupid files a read every sixth turn."
      placement="top-center"
      messageClassName="text-aura-ink"
    >
      <GaugeColumn>
...
function JudgeGauge({ passes }: { passes: number }) {
  ...
  return (
    <Tooltip
      message="Filed Cupid reads. Each read logs how the date is going."
      placement="top-center"
      messageClassName="text-aura-ink"
    >
      <GaugeColumn>
...
function NudgeGauge({
  ...
  const tooltip = enabled
    ? "Whisper one private nudge to a dater."
    : remaining === 0
      ? "All nudges have been filed."
      : "Pause the date before whispering a nudge.";

  return (
    <Tooltip message={tooltip} placement="top-center" messageClassName="text-aura-ink">
      <button
        ref={buttonRef}
        type="button"
    ...
    <GaugeColumn>
      ...
      <span className="inline-flex items-center gap-1">
        {picks.map((eventId) => {
          ...
          const tooltip = dropped
            ? `${tactic}: ${title}. Already dropped into this date.`
            : interactive
              ? `${tactic}: ${title}. Preview and drop this prepared scene.`
              : `${tactic}: ${title}. Pause the date before dropping a scene.`;
          return (
            <Tooltip
              key={eventId}
              message={tooltip}
